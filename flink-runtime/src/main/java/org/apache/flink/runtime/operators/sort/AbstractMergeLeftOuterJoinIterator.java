 ...
 */
public abstract class AbstractMergeLeftOuterJoinIterator<T1, T2, O> extends AbstractMergeIterator<T1, T2, O> {
	...
	public AbstractMergeLeftOuterJoinIterator(
	 ...
	 */
	@Override
	public boolean callWithNextKey(final FlatJoinFunction<T1, T2, O> matchFunction, final Collector<O> collector) throws Exception {
		if (!initialized) {
	...
	private void joinLeftKeyValuesWithNull(Iterator<T1> values, FlatJoinFunction<T1, T2, O> matchFunction, Collector<O> collector) throws Exception {
		while (values.hasNext()) {
			T1 next = values.next();
			this.copy1 = createCopy(serializer1, next, copy1);
			matchFunction.join(copy1, null, collector);
		}
	}

	private void joinRightKeyValuesWithNull(Iterator<T2> values, FlatJoinFunction<T1, T2, O> matchFunction, Collector<O> collector) throws Exception {
		while (values.hasNext()) {
			T2 next = values.next();
			this.copy2 = createCopy(serializer2, next, copy2);
			matchFunction.join(null, copy2, collector);
		}
	}
